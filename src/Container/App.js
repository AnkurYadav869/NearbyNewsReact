...
function App() {
	...
	useEffect(() => {
		...
		// DONE....
		fetch(
			'http://api.weatherapi.com/v1/forecast.json?key=28ac2907b80d44e487c65115211003&q=Hyderabad&days=1&aqi=no&alerts=no'
		)
			.then((res) => res.json())
			.then((data) => {
				console.log(
			...
			.catch(console.log('ERROR main'));
	}, []);

	const changeCity = (city) => {
		fetch(
			`http://api.weatherapi.com/v1/forecast.json?key=28ac2907b80d44e487c65115211003&q=${city}&days=1&aqi=no&alerts=no`
		)
			.then((res) => res.json())
			.then((data) => {
